...
def _build_entry(snap: dict, sig: dict, all_pos: list) -> dict:
    ...
    for entry in all_pos:
        ...
        try:
            for _, r in df.iterrows():
                ...
                side     = str(r.get("side", ""))
                coin     = str(r.get("coin", ""))
                upnl_pct = float(r.get("upnl_pct", (upnl / notional * 100) if notional else 0))
                if entry_px <= 0:
                    continue

                dist_pct = None
                if coin == "BTC" and btc_now:
                    dist_pct = ((btc_now - entry_px) / entry_px * 100 if side == "LONG" else (entry_px - btc_now) / entry_px * 100)

                liq_price = None
                if lev > 1:
                    liq_price = round(entry_px * (1 - 0.9 / lev) if side == "LONG" else entry_px * (1 + 0.9 / lev), 2)
    ...
    return {
        ...
        "excluded_mm_list": [
            {"display": m["display"], "ratio": m.get("ratio", 0), "n_pos": m.get("n_pos", 0)}
...
def _rank_by_consistency(whale_positions: list) -> list:
    ranked = sorted(whale_positions, key=lambda w: w.get("consistency_score", 0), reverse=True)
    result = []
    for i, w in enumerate(ranked[:15]):
        ...
        result.append({
            ...
            "top_coins": [
                ...
                for pos in sorted(w.get("positions", []), key=lambda x: x["notional"], reverse=True)[:3]
            ],
        })
    return result
...
def _compute_entry_clusters(whale_positions: list, btc_now: float) -> list:
    ...
    for w in whale_positions:
        ...
        for p in w.get("positions", []):
            coin, side, entry, not_ = p["coin"], p["side"], p["entry_px"], p["notional"]
            if entry <= 0:
                continue
            coin_data[coin][side.lower()].append({"entry": entry, "notional": not_, "qs": qs, "cs": cs, "display": w["display"]})
    ...
    for coin, sides in coin_data.items():
        for side_key, positions in sides.items():
            ...
            clusters.append({
                ...
                "total_notional": round(total_not, 0),
                "n_whales": n_whales,
                "quality_score": round(avg_qs, 3),
                "consistency_score": round(avg_cs, 1),
                "zone_type": zone_type,
                "btc_dist_pct": round(dist, 2) if dist is not None else None,
                "top_whales": [p["display"] for p in sorted(positions, key=lambda x: x["notional"], reverse=True)[:3]],
            })
    clusters.sort(key=lambda x: x["total_notional"], reverse=True)
    return clusters[:12]
